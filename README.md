...
> **Estado: parcialmente implementado (coordenação validada; montagem ainda não fecha ao vivo).** O **coordenador** foi implementado e **validado ao vivo**: reconhece tasks de 2 blocos, atribui papéis (*submitter*/*helper*), escolhe a goal zone de montagem e anuncia (`tarefa_multi`). O **protocolo do worker** (busca → *rendezvous* → barreira → `connect` → `detach`/`submit`) está **escrito e compila**, mas **não fecha ao vivo**. A depuração ao vivo descascou três camadas de dificuldade, todas características do domínio:
>
> 1. **Posicionar-se no dispenser (resolvido):** o agente re-mirava no dispenser visível mais próximo a cada passo e **derivava** ~100 células sem encaixar. Corrigido com *commit-to-dispenser* — o agente fixa um alvo (`disp_alvo`, a posição absoluta própria do dispenser) e navega até o offset; com alvo fixo, **converge** (posição passou a ficar limitada).
> 2. **Travamento em `skip` (resolvido):** quando nenhum plano da fase se aplicava, o agente travava. Corrigido com salvaguarda (libera o papel e explora) e *fallback* que explora em vez de `skip`.
> 3. **Ciclo de vida da task (bloqueio remanescente):** a task anunciada **expira antes** de os agentes montarem — a navegação de *rendezvous* no toro é lenta e o cenário tem alta rotatividade de tasks, então cada task escolhida some antes da montagem (num teste, a salvaguarda de expiração disparou 164×). A fase de `connect` sincronizado **não chegou a ser exercida**.
>
> Em suma, fechar a montagem cooperativa ao vivo exige conjugar **posicionamento + sincronização + ciclo de vida da task + velocidade de navegação no toro** — é **trabalho futuro de escopo de pesquisa**, exatamente o risco antecipado abaixo. Por isso o multi-bloco fica **desativado por padrão** (`flag_multibloco`), para que no `sim2` do SampleConfig (todo multi-bloco) os agentes **explorem normalmente** em vez de operar o protocolo incompleto. O pipeline validado (D2) cobre tarefas de **1 bloco**.
